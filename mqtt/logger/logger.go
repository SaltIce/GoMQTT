...
type logger struct {
	InfoOpen bool
	DebugOpen bool
	Level Logger.LogMode // #LogMode = 0 //默认   LogMode = 1 //只输出main文件    LogMode = 2 //只出输cmd
	LogPath string
}
//获取日志配置，防止循环依赖
func ReadLogConfig(t *Log, filePath string) (*Log, error) {
	data, err := ioutil.ReadFile(filePath)
	if err!= nil{
		Logger.PError(err,getfile()+"读取配置文件"+filePath+"出错")
		return nil,err
	}
	//把yaml形式的字符串解析成struct类型 t保存初始数据
	err = yaml.Unmarshal(data, t)
	if err!=nil{
		Logger.PError(err,getfile()+"解析配置文件"+filePath+"出错")
		return nil,err
	}
	return t,nil
	//d, _ := yaml.Marshal(&t)
...
func init() {
	co := Log{}
	consts , err := ReadLogConfig(&co,"mqtt/config/logger.yml")
	if err!=nil{
		panic(err)
	}
	InfoOpen = consts.Logger.InfoOpen
	DebugOpen = consts.Logger.DebugOpen
	if !InfoOpen{
	...
	level=consts.Logger.Level
	if consts.Logger.LogPath != ""{
		logPath=consts.Logger.LogPath
	}
}
