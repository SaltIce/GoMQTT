...
var (
	defaultTopicsProvider = ""
	ServerTopicProvider2 = " "
	memTOpen bool
)
func init() {
	co := config.MyConst{}
	...
	ServerTopicProvider2 = consts.ServerConf.TopicsProvider
	if ServerTopicProvider2 == defaultTopicsProvider{
		memTOpen = true
	}
}
func init() {
	if defaultTopicsProvider!=""{
		if memTOpen{
			Register(defaultTopicsProvider, NewMemProvider())
		}
	}else {
		fmt.Println("获取默认DefaultTopicsProvider失败")
	}
}
