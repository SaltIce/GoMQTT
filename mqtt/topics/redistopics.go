...
func init() {
	...
	redisTopicsProvider = consts.MyConst[0].Nodes[0].RedisTopicsProvider
	ServerTopicsProvider = consts.ServerConf.TopicsProvider
	if ServerTopicsProvider == redisTopicsProvider{
		redisTOpen = true
	}
}
func init() {
	if redisTopicsProvider!=""{
		if redisTOpen{
			Register(redisTopicsProvider, NewRedisProvider())
		}
	}else {
		fmt.Println("获取默认RedisTopicsProvider失败")
	}
}
...
func (this *redisTopics) Subscribers(topic []byte, qos byte, subs *[]interface{}, qoss *[]byte) error {
	if !message.ValidQos(qos) {
