...
const (
	defaultBufferSize     = 1024 * 256 //comment.DefaultBufferSize 通过这样赋值不行，就算改为var也不行，所以还是写死在这里面吧，或者再读取配置文件获取
	defaultReadBlockSize  = 8192       //comment.DefaultReadBlockSize
	defaultWriteBlockSize = 8192       //comment.DefaultWriteBlockSize
)
