...
type service struct {
	...
	clientLinkPub func(msg interface{}) error // 当前服务连接到其它服务的连接处理方法，发送普通消息给其它节点使用
}
...
func (this *service) stop() {
	...
	if !this.client && this.sess != nil {
		...
		if err != nil {
			logger.Errorf(err, "(%s/%d): %v", this.cid(), this.id, err)
		} else {
			for _, t := range tpc {
				if err := this.topicsMgr.Unsubscribe([]byte(t), &this.onpub); err != nil {
					logger.Errorf(err, "(%s): Error unsubscribing topic %q: %v", this.cid(), t, err)
				}
			}
		}
	}
