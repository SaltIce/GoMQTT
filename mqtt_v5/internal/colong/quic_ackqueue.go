...
type ackmsg struct {
	...
	// Packet ID of the  Every message that require ack'ing must have a valid
...
// ack消息被接收。
func (this *Ackqueue) Wait(msg Message, onComplete interface{}) error {
	this.mu.Lock()
...
func (this *Ackqueue) Ack(msg Message) error {
	...
	case PINGRESP:
		if this.ping.Mtype == PINGREQ {
			this.ping.State = PINGRESP
		}
...
FORNOTEMPTY:
	for !this.empty() {
		...
		case MQ_TAG_CLU, PUBACK, PUBREL, PUBCOMP, SUBACK, UNSUBACK:
...
func (this *Ackqueue) insert(pktid uint16, msg Message, onComplete interface{}) error {
	if this.full() {
		this.grow()
	}
