...
// publish转sys的
func (this *header) decodePubToSys(src []byte) (int, error) {
	total := 0
	...
	if this.Type() == PUBLISH && !ValidQos((this.Flags()>>1)&0x3) {
		return total, fmt.Errorf("header/decodeToSys: Invalid QoS (%d) for PUBLISH message.", (this.Flags()>>1)&0x3)
	}
	this.SetType(mtype)
	total++
...
func (this *header) decodePubToShare(src []byte) (int, error) {
	...
	// publish -> sys
	if mtype != SHAREPUBLISH && PUBLISH != this.Type() {
		return total, fmt.Errorf("header/Decode: Invalid message type %d. Expecting %d.", this.Type(), mtype)
	}
