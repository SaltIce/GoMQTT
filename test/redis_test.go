...
func init_redis() {
	redisSUrl := "10.112.26.131:6379"
	redisSPassword := "root"
	redisSDB := 1
	if isEmpty(redisSUrl, redisSPassword) {
		panic("redis info have empty")
	}
	R = redis.NewClient(&redis.Options{DB: int(redisSDB), Addr: redisSUrl, Network: "tcp"})
}
...
func TestRedis(t *testing.T) {
	...
	SubShare("sk", "pp2", "sb")
	SubShare("sk", "pp2", "sb")
	fmt.Println(GetTopicShare("sk"))
	fmt.Println(DelTopic("sk"))
}
